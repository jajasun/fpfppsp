...
public class ElementLeaderManager : Singleton<ElementLeaderManager>
{
    ...
    public Transform Calc(Vector3 pos)
    {
        return brain.Calc(pos);
    }
    ...
    static float[,] TrainingData = new float[,]
{
    ...
    {
        // 距離を計算する
        Zone[] zone = ElementLeaderManager.Instance._zone;
        int zoneNum = zone.Length;
        float[] distance = new float[zoneNum];
        for (int i = 0; i < zoneNum; i++)
        {
            Vector3 t = pos - zone[i].transform.position;
            distance[i] = Mathf.Abs(t.magnitude);
            distance[i] /= 100;
        }
        Neuron.SetInput(0, distance[0]);
        Neuron.SetInput(1, distance[1]);
        ...
        // 敵味方のレート計算(ゾーンの外側から取得する)
        Team team = Team.Red;

        int num1 = (int)Team.Red;
        int num2 = (int)Team.Blue;
        ...
        Neuron.SetInput(5, rate[0]);
        Neuron.SetInput(6, rate[1]);
        Neuron.SetInput(7, rate[2]);
        Neuron.SetInput(8, rate[3]);
        Neuron.SetInput(9, rate[4]);
        ...
        float[] otherTarget = new float[zoneNum];
        for (int i = 0; i < zoneNum; i++)
        {
            float target = targetZone[i];
            target /= 16;
            otherTarget[i] = target;
        }
        Neuron.SetInput(10, otherTarget[0]);
        Neuron.SetInput(11, otherTarget[1]);
        Neuron.SetInput(12, otherTarget[2]);
        Neuron.SetInput(13, otherTarget[3]);
        Neuron.SetInput(14, otherTarget[4]);
        ...
        switch (Neuron.GetMaxOutputID())
        {
            case 0:
                // A地点
                Debug.Log("Target : A");
                return zone[0].transform;
            case 1:
                // B地点
                Debug.Log("Target : B");
                return zone[1].transform;
            case 2:
                // C地点
                Debug.Log("Target : C");
                return zone[2].transform;
            case 3:
                // D地点
                Debug.Log("Target : D");
                return zone[3].transform;
            case 4:
                // E地点
                Debug.Log("Target : E");
                return zone[4].transform;
            default:
                return null;
